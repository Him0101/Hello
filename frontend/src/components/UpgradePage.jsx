...
import { Check, Crown, Zap, Globe, Headphones, FileText, MessageSquarePlus } from "lucide-react";
      ...
      <div className="max-w-4xl mx-auto px-5 md:px-10 py-8">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
          ...
          <h1 className="font-heading text-2xl md:text-3xl font-bold tracking-tight text-zinc-900">
            Unlock the full power of Sarvbhasa
          </h1>
          <p className="font-body text-sm text-zinc-500 mt-2 max-w-md mx-auto">
            Get unlimited translations, more languages, and advanced features
          </p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            ...
            <div className="space-y-3 mb-6">
              {FREE_FEATURES.map((f, i) => {
                const Icon = f.icon;
                return (
                  <div key={i} className="flex items-center gap-3 text-[13px] font-body text-zinc-600">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
            ...
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full bg-saffron text-white text-[10px] font-body font-bold uppercase tracking-wider">
              Recommended
            </div>
            ...
            <Button className="w-full rounded-full bg-gradient-to-r from-saffron to-india-green hover:opacity-90 text-white font-body text-[13px] font-medium"
              data-testid="upgrade-pro-btn">
              Upgrade to Premium
            </Button>
            <p className="text-center text-[10px] font-body text-zinc-400 mt-2">Coming Soon — Payment integration</p>
          </motion.div>
        </div>
      </div>
    </div>
