...
export default function SpeechToTextPage() {
  ...
  const startListening = () => {
    ...
    const r = new SR(); r.lang = language; r.interimResults = true; r.continuous = true;
    r.onresult = (e) => {
      let final = "", interim = "";
  ...
  return (
    <div className="h-full overflow-y-auto wavy-tricolor-bg" data-testid="speech-to-text-page">
      <div className="relative z-10 max-w-3xl mx-auto px-5 md:px-10 py-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-8">
          ...
          <Select value={language} onValueChange={setLanguage}>
            <SelectTrigger className="w-[180px] rounded-lg border-zinc-200 bg-white/80 backdrop-blur-sm font-body text-[13px]" data-testid="speech-language-select"><SelectValue /></SelectTrigger>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="flex flex-col items-center mb-8">
          <div className="relative mb-4">
            ...
            <button data-testid="speech-mic-btn" onClick={isListening ? stopListening : startListening}
              className={`relative w-20 h-20 rounded-full flex items-center justify-center transition-all ${isListening ? "bg-red-500 hover:bg-red-600 shadow-lg shadow-red-200" : "bg-gradient-to-br from-saffron to-india-green hover:opacity-90 shadow-lg"}`}>
          ...
          <p className="font-body text-[13px] font-medium text-zinc-500">
            {isListening ? <span className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /> Listening... Click to stop</span> : "Start Speaking"}
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-1">
              ...
              <button onClick={() => { setTranscript(""); setInterimText(""); }} disabled={!transcript} className="p-1 rounded hover:bg-zinc-100 transition-colors disabled:opacity-30" data-testid="speech-clear-btn">
                <Trash2 className="w-3.5 h-3.5 text-zinc-400" />
              </button>
            </div>
          </div>
          <div className="min-h-[180px] rounded-xl border border-zinc-200 bg-white/80 backdrop-blur-sm p-4 font-body text-[13px] text-zinc-700 leading-relaxed" data-testid="speech-transcript-area">
            {transcript || interimText ? <>{transcript}{interimText && <span className="text-zinc-400 italic">{interimText}</span>}</> : <span className="text-zinc-300">Your transcript will appear here...</span>}
          </div>
        </motion.div>
      </div>
    </div>
