...
function openGooglePopup() {
  ...
  window.open(authUrl, "google-auth", `width=${w},height=${h},left=${left},top=${top},popup=1`);
}
...
export default function AuthModal({ onClose, onSuccess, defaultTab = "login" }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) setError(formatError(data.detail));
      else onSuccess(data);
  ...
  const handleGoogleLogin = () => {
    ...
    openGooglePopup();
    // The popup posts message back via auth-callback.html -> App.js listener handles it
    const checkClosed = setInterval(async () => {
      // Also check if auth completed via cookie
      try {
    ...
    setTimeout(() => { clearInterval(checkClosed); setSocialLoading(""); }, 60000);
  };
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="space-y-2">
            <button type="button" data-testid="auth-google-btn" onClick={handleGoogleLogin} disabled={!!socialLoading}
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg border border-zinc-200 hover:bg-zinc-50 transition-colors text-[13px] font-body font-medium text-zinc-700 disabled:opacity-50">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button type="button" data-testid="auth-twitter-btn"
                className="flex items-center justify-center gap-1.5 py-2.5 rounded-lg border border-zinc-200 hover:bg-zinc-50 transition-colors text-[12px] font-body font-medium text-zinc-600">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-body text-zinc-400 uppercase">or with email</span>
            <div className="flex-1 h-px bg-zinc-100" />
          </div>
